...
pub const Swapchain = struct {
    ...
    pub fn init(gc: *const GPU, allocator: std.mem.Allocator, extent: vk.Extent2D) !Swapchain {
        return try initRecycle(gc, allocator, extent, .null_handle);
    }

    pub fn initRecycle(gc: *const GPU, allocator: std.mem.Allocator, extent: vk.Extent2D, old_handle: vk.SwapchainKHR) !Swapchain {
...
const SwapImage = struct {
    ...
    fn init(gc: *const GPU, image: vk.Image, format: vk.Format) !SwapImage {
    ...
    fn deinit(self: SwapImage, gc: *const GPU) void {
    ...
    fn waitForFence(self: SwapImage, gc: *const GPU) !void {
        _ = try gc.dev.waitForFences(1, @ptrCast(&self.frame_fence), .true, std.math.maxInt(u64));
    }
};

fn initSwapchainImages(gc: *const GPU, swapchain: vk.SwapchainKHR, format: vk.Format, allocator: std.mem.Allocator) ![]SwapImage {
...
fn findSurfaceFormat(gc: *const GPU, allocator: std.mem.Allocator) !vk.SurfaceFormatKHR {
...
fn findPresentMode(gc: *const GPU, allocator: std.mem.Allocator) !vk.PresentModeKHR {
